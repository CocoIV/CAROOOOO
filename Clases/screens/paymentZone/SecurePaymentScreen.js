...
const SecurePaymentScreen = ({ navigation, route }) => {
  ...
  const handlePayment = async () => {
    ...
    // No almacenar datos de tarjeta, solo usarlos para el checkout
    // Generar URL de checkout con parámetros (puedes usar tu backend para esto)
    const url = `${PAYMENT_CHECKOUT_URL}?cedula=${encodeURIComponent(cedula)}&nombre=${encodeURIComponent(nombre)}&monto=${encodeURIComponent(monto)}`;
    try {
      await Linking.openURL(url);
      Alert.alert('Redirigido', 'Se abrió el checkout seguro en el navegador.');
      // Limpiar datos temporales
      setCardNumber('');
      setCardExp('');
      setCardCVC('');
    } catch (err) {
      Alert.alert('Error', 'No se pudo abrir el checkout.');
    }
    setLoading(false);
  };
